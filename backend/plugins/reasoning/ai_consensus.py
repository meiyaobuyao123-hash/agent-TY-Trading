...
MARKET_TYPE_CONTEXT = {
    "crypto": (
        "这是加密货币市场，24/7全天候交易，波动性较高。"
        "注意BTC的联动效应——如果不是BTC本身，需要考虑BTC走势对该币的影响。"
        "\n\n"
        "【极其重要 — 4小时预测窗口校准(准确率仅15%，必须严格执行以下规则！)】\n"
        "历史数据显示: 在4小时窗口内，加密货币70%的时间价格变化在1.5%以内(即flat)。\n"
        "你之前的预测中，62%预测了'down'，但实际只有不到15%是正确的！\n"
        "核心问题：你严重高估了短期内价格大幅变动的概率。\n\n"
        "【强制执行的交易规则 — 违反任何一条都会降低准确率】\n"
        "规则1 — 震荡市(默认状态): 如果24h涨跌幅在-2%到+2%之间，且RSI在30-70之间，\n"
        "  必须预测flat，confidence=0.45，概率分布: up=0.28, down=0.27, flat=0.45\n"
        "  这是最安全的选择，历史数据证明70%的4h窗口结果都是flat。\n\n"
        "规则2 — RSI超买反转: 如果RSI>70，预测down(均值回归)，confidence=0.45\n"
        "  概率分布: up=0.20, down=0.45, flat=0.35\n\n"
        "规则3 — RSI超卖反弹: 如果RSI<30，预测up(均值回归)，confidence=0.45\n"
        "  概率分布: up=0.45, down=0.20, flat=0.35\n\n"
        "规则4 — 强动量(罕见): 只有当24h涨跌幅>5%时，才预测趋势延续方向\n"
        "  涨>5%: 预测up, confidence=0.5\n"
        "  跌>5%: 但要注意均值回归 — 跌>5%实际上反弹概率更高，预测up\n\n"
        "规则5 — 弱信号区(2%-5%变化): 仍然倾向flat\n"
        "  flat_probability应>=0.40，不要轻易选择方向\n\n"
        "规则6 — 绝不因为微跌(-2%到0%)就预测继续跌！这是你最大的错误来源。\n"
    ),
    "us-equities": (
